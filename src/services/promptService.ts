...
export function generateTechnicalPrompt(config: BackgroundConfig, framework: 'react' | 'vanilla'): string {
  ...
  const base = framework === 'react' 
    ? `Act as a senior frontend engineer. Create a high-performance, production-ready React component for an animated background effect using Motion Lab specs. The component must be self-contained, responsive, and optimized for 60fps.`
    : `Act as a senior frontend engineer. Create a high-performance, production-ready Vanilla JavaScript and CSS implementation for an animated background effect using Motion Lab specs. The code must be self-contained, responsive, and optimized for 60fps.`;
...
- Colors: Use ${allColors}. Apply gradients across each ribbon for a rich, textured look.`,
    ...
    bokeh: `
- Visual: A high-fidelity "Bokeh" background simulating depth of field.
- Implementation: Use multiple 'motion.div' elements representing out-of-focus light circles.
- Logic: Calculate blur intensity based on each circle's distance from a "focus plane" (${config.customSettings?.focusPlane ?? 0.5}).
- Customization: Support adjustable focus plane, aperture (${config.customSettings?.aperture ?? 0.2}), shape (circle, hexagon, square), and glow intensity (${config.customSettings?.glow ?? 0.8}).
- Animation: Circles drift and scale slowly based on speed (${speed}).
- Colors: Use ${allColors}. Use 'screen' blend mode for realistic light additive effects.`,

    avatars: `
- Visual: A modern "Glassmorphism Avatar" background with floating icons.
- Implementation: Use multiple 'motion.div' elements with 'backdrop-filter: blur()' and semi-transparent backgrounds.
- Logic: Each element contains a Lucide icon (${config.customSettings?.iconType || 'user'}). Elements float and rotate organically.
- Customization: Support adjustable blur intensity (${config.customSettings?.blur ?? 10}px), frost (opacity: ${config.customSettings?.frost ?? 0.2}), shape (circle/square), and icon type.
- Animation: Elements drift and scale based on speed (${speed}). Includes a background mesh layer for color depth.
- Interactivity: ${interactive ? `Elements respond to mouse movement with a parallax factor based on their Z-depth.` : 'Static floating elements.'}
- Colors: Use ${allColors} for the icons and the background mesh layer.`,
  };
...
export const MotionLabBackground = () => {
  ...
  useEffect(() => {
    ...
    // Full implementation for ${config.type} — see Motion Lab source on GitHub.
  }, []);
